...
export function baseImgUrl(url: any) {
  let config = useConfigStore();
  return url.includes("http") ? url : config.baseImgUrl + url;
}
 ...
 * @param data 数据
 * @param data.x x轴数据
 * @param data.series[] 数据
 * @param data.series[][] [0] 数据标题 [1] 基础数据
 * @returns
 */
export const objColumn = (title: string, data: { x: string[]; series: any[][] }): ECOption => {
  return {
    title: {
      text: title
    },
 ...
 * @returns
 */
export const objLine = (title: string, data: { x: string[]; series: any[][] }): ECOption => {
  return {
    title: {
      text: title
    },
...
// 时间筛选
export const timeSiftFun: any = (): any => {
  return [
    {
      prop: "startDate",
      label: "开始时间",
