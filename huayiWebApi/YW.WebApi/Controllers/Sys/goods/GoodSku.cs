...
{
    /// <summary>
    /// 控制器层 SysGoodSkuController
    /// </summary>
    public class SysGoodSkuController : BaseController
    {

        private readonly IGoodSkuService _goodSkuService;
        private readonly IGoodsService _goodsService;

        private readonly GoodSkuMapper mapper = new();
        public SysGoodSkuController(IClaimsAccessor claimsAccessor,
            GoodSkuService goodSkuService, GoodsService goodsService)
        {
        ...
        public async Task<ResultModel> List(QueryModel queryModel)
        {
            ...
            var exWhere = PredicateBuilder.New<GoodSku>();
            ...
            {
                exWhere.And(a => a.goodsId == queryModel.queryId.Value);
            }
            if (!string.IsNullOrWhiteSpace(queryModel.queryName))
            {
                ...
                else exWhere.And(a => a.name.Contains(queryModel.queryName) || a.url.Contains(queryModel.queryName));
            ...
            var data = await _goodSkuService.GetPageListAsync(exWhere, p, it => new { it.sort, it.createTime, it.Id }, OrderByType.Desc);
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ResultModel> Operation(GoodSkuView model)
        {
            var res = new ResultModel();
            ...
            if (info.Id > 0)
            {
                ...
                isok = await _goodSkuService.UpdateAsync(info);
            }
            else
            {
                info.createTime = DateTime.Now;
                info.updateTime = DateTime.Now;
                isok = await _goodSkuService.InsertAsync(info);
            }
        ...
        public async Task<ResultModel> Delete(DelModel del)
        {
            ...
            var isok = await _goodSkuService.UpdateAsync(it => new GoodSku { status = 99 }, it => SqlFunc.ContainsArray(del.ids, it.Id));
