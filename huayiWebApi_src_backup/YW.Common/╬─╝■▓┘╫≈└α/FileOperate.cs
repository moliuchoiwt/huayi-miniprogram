...
namespace YW.Common
{
    public class FileOperate
    {
        ...
        protected void Write_Txt(string FileName, string Content)
        {
            ...
            string htmlfilename = CommonHelper.GetMapPath("Precious\\" + FileName + ".txt");//HttpContext.Current.Server.MapPath("Precious\\" + FileName + ".txt");　//保存文件的路径
            string str = Content;
        ...
        protected string Read_Txt(string filename)
        {
            ...
            Encoding code = Encoding.GetEncoding("gb2312");
            string temp = CommonHelper.GetMapPath("Precious\\" + filename + ".txt");//HttpContext.Current.Server.MapPath("Precious\\" + filename + ".txt");
            string str = "";
            if (File.Exists(temp))
            {
