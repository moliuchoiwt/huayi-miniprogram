...
{
    /// <summary>
    /// 控制器层 SysLikesController
    /// </summary>
    public class SysLikesController : BaseController
    {

        private readonly ILikesService _likesService;

        private readonly LikesMapper mapper = new();
        public SysLikesController(IClaimsAccessor claimsAccessor, LikesService likesService)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ResultModel> Operation(LikesView model)
        {
            var res = new ResultModel();
            ...
            if (info.Id > 0)
            {
                ...
                isok = await _likesService.UpdateAsync(info);
            }
            else
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ResultModel> DelLikes(DelModel del)
        {
            var res = new ResultModel();
            ...
            var isok = await _likesService.UpdateAsync(it => new Likes { State = 99 }, it => SqlFunc.ContainsArray(del.ids, it.Id));
