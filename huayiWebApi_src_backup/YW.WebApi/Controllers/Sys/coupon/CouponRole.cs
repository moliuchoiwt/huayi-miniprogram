...
{
    /// <summary>
    /// 控制器层 SysCouponRoleController
    /// </summary>
    public class SysCouponRoleController : BaseController
    {

        private readonly ICouponRoleService _couponRoleService;

        private readonly CouponRoleMapper mapper = new();
        public SysCouponRoleController(IClaimsAccessor claimsAccessor, CouponRoleService couponRoleService)
        {
        ...
        public async Task<ResultModel> List(QueryModel queryModel)
        {
            ...
            var exWhere = PredicateBuilder.New<CouponRole>();
            ...
            var data = await _couponRoleService.GetPageListAsync(exWhere, p, it => new { it.CreateTime, it.Id }, OrderByType.Desc);
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ResultModel> Operation(CouponRoleView model)
        {
            var res = new ResultModel();
            ...
            if (info.Id > 0)
            {
                ...
                isok = await _couponRoleService.UpdateAsync(info);
            }
            else
            {
                info.CreateTime = DateTime.Now;
                info.UpdateTime = DateTime.Now;
                isok = await _couponRoleService.InsertAsync(info);
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ResultModel> DelCouponRole(DelModel del)
        {
            var res = new ResultModel();
            ...
            var isok = await _couponRoleService.UpdateAsync(it => new CouponRole { State = 99 }, it => SqlFunc.ContainsArray(del.ids, it.Id));
